...
> 适配器已按本文实现，这些结论也被编码进了合成 fixtures（`agent_chat_viewer/demo.py`）——
> 也就是说，测试证明"**如果**落盘格式如本文所述，适配器就能正确归一化"，但不能证明本文描述得对。
>
> 要在一台装了真实数据的机器上核对，跑 `just probe --fields --sample 2`：它用与适配器相同的
> 只读快照路径打开各个库，打印实际出现的 line type / key 前缀 / 字段直方图 / 样本记录。
> 与本文不符的地方，先改本文和 fixtures，再改适配器。
